...
static int __devinit xgpio_of_probe(struct device_node *np)
{
    ...
    /* Update GPIO2 state shadow register with default value */
	  ...
	  if (tree_info)
	  	chip->gpio2_state = be32_to_cpup(tree_info);
  }
    ...
    /* Update GPIO2 direction shadow register with default value */
	  ...
	  if (tree_info)
	  	chip->gpio2_dir = be32_to_cpup(tree_info);
  }
	...
	if (tree_info)
	{
		ofchip->gc.ngpio = be32_to_cpup(tree_info);
		chip->gpio_width = be32_to_cpup(tree_info);
	}
	...
	if (chip->isdual)
	{
    	...
    	{             
       	ofchip->gc.ngpio += be32_to_cpup(tree_info);
        chip->gpio2_width = be32_to_cpup(tree_info);
      }    	
  }
